    ...
    <div className="min-h-screen pt-24 pb-16">
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        <motion.div
          ...
          className="relative flex gap-1 bg-dark-900 rounded-xl p-1 border border-dark-700 mb-6 overflow-x-auto scrollbar-hide"
        >
          {tabs.map(t => (
            <button
              key={t.value}
        ...
        {loading ? <PageLoader /> : bookings.length === 0 ? (
          <motion.div
            variants={fadeUp} initial="hidden" animate="show" transition={easeOut}
            className="text-center py-20 text-dark-400"
          >
            <div className="w-16 h-16 mx-auto mb-4 opacity-30 animate-float">
                ...
                {bookings.map(b => {
                  ...
                  return (
                    <motion.div key={b.id} variants={staggerItem} layout>
                      <Link to={`/bookings/${b.id}`} className="card-hover block p-4 group">
                        <div className="flex gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-2">
                              <h3 className="font-semibold text-white line-clamp-1 group-hover:text-primary-300 transition-colors">{b.movieTitle}</h3>
